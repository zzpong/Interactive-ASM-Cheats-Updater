code_pattern = {
    "loc_EN":
    {
        ...
        "code_type_0xFFF":
        {
            "pattern": "r'^ *FFF[1248][abcdef\d][01234][abcdef\d]{2} *( * [abcdef\d]{8}){0,1} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0xFFF: Debug Log',
            "details":
                """[f'Code type 0xFFF writes a debug log to the SD card.']"""
        },
        "code_type_unknown":
        {
    ...
    "loc_CN":
        ...
        "code_type_0x0":
        {
            ...
            "details":
                """[f'Code type 0x0 会向指定内存地址写入数据。']"""
        },
        "code_type_0x1":
        {
            "pattern": "r'^ *1[1248][0123][123456]00[abcdef\d]{2} *( * [abcdef\d]{8}){2,3} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0x1: 条件循环开始标志',
            "details":
                """[f'Code type 0x1 为条件循环开始标志，它会将特定内存数据与给定常量比较。',
                    '如果条件不满足，则跳过此指令至 Code type 0x2 之间的所有指令。']"""
        },
        "code_type_0x2":
        {
            "pattern": "r'^ *2[01]000000 *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0x2: 条件循环结束标志',
            "details":
                """[f'Code type 0x2 为条件循环结束标志 (循环始于 Code Type 0x1 或 Code Type 0x8)。',
                    '在使用 else 指令时，此结束标志前的所有代码都将被跳过。']"""
        },
        "code_type_0x3":
        {
            "pattern": "r'^ *3[01]0[abcdef\d]0000 *( * [abcdef\d]{8}){0,1} *$'",
            ...
            "details":
                """[f'Code type 0x3 将执行指定次数的循环体内指令。']"""
        },
        "code_type_0x4":
        {
            "pattern": "r'^ *400[abcdef\d]0000 *( * [abcdef\d]{8}){2} *$'",
            "generate_type": 'force_generate',
            "description": 'Code Type 0x4: 常量数据载入寄存器',
            "details":
                """[f'Code type 0x4 会向寄存器写入指定常量数据。']"""
        },
        "code_type_0x5":
        {
            "pattern": "r'^ *5[1248][0123][abcdef\d][01]0[abcdef\d]{2} * [abcdef\d]{8} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0x5: 内存地址数据载入寄存器',
            "details":
                """[f'Code type 0x5 会向寄存器写入指定内存地址数据。']"""
        },
        "code_type_0x6":
        {
            "pattern": "r'^ *6[1248]0[abcdef\d][01][01][abcdef\d]0 *( * [abcdef\d]{8}){2} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0x6: 常量数据写入内存',
            "details":
                """[f'Code type 0x6 会向寄存器指向地址写入常量数据。']"""
        },
        "code_type_0x7":
        {
            "pattern": "r'^ *7[1248]0[abcdef\d][01234]000 * [abcdef\d]{8} *$'",
            ...
            "details":
                """[f'Code type 0x7 为寄存器算术运算指令。',
                    '该指令已被 Code type 0x9 取代，目前仅为遗留功能。']"""
        },
        "code_type_0x8":
        {
            "pattern": "r'^ *(8)[abcdef\d]{7} *$'",
        ...
        "code_type_0xA":
        {
            ...
            "details":
                """[f'Code type 0xA 会将寄存器数据写入指定内存地址。']"""
        },
        "code_type_0xC0":
        {
            "pattern": "r'^ *C0[1248][123456][abcdef\d][012345][abcdef\d]{2} *( * [abcdef\d]{8}){0,2} *$'",
        ...
        "code_type_0xC1":
        {
            ...
            "details":
                """[f'Code type 0xC1 提供保存或恢复寄存器数据的功能。']"""
        },
        "code_type_0xC2":
        {
            "pattern": "r'^ *C2[0123]0[abcdef\d]{4} *$'",
            ...
            "details":
                """[f'Code type 0xC2 提供通过掩码同时保存或恢复多个寄存器数据的功能。']"""
        },
        "code_type_0xC3":
        {
            "pattern": "r'^ *C3000[078F][0F][abcdef\d] *$'",
            ...
            "details":
                """[f'Code type 0xC3 将使用指定寄存器读/写某静态寄存器。']"""
        },
        "code_type_0xF0":
        {
            "pattern": "r'^ *F0[abcdef\d]{6} *$'",
        ...
        "code_type_0xFF0":
        {
            "pattern": "r'^ *FF0[abcdef\d]{5} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0xFF0: 暂停进程',
            "details":
                """[f'Code type 0xFF0 暂停当前进程。']"""
        },
        "code_type_0xFF1":
        {
            "pattern": "r'^ *FF1[abcdef\d]{5} *$'",
            "generate_type": 'flat_generate',
            "description": 'Code Type 0xFF1: 恢复进程',
            "details":
                """[f'Code type 0xFF1 恢复当前进程。']"""
        },
        "code_type_0xFFF":
        {
            "pattern": "r'^ *FFF[1248][abcdef\d][01234][abcdef\d]{2} *( * [abcdef\d]{8}){0,1} *$'",
    ...
    "loc_EN":
        {
            ...
            "hints_map":
            {
                ...
                "Input Old Codes with BID:": """[f'Old Codes <BID {self.old_main_file.ModuleId.upper()}>:']""",
                "Current Processing Codes:": "Current Processing Codes:",
                "Wing Length:": "Wing Length:",
                "Logs:": "Logs:",
                "New Codes Output:": "New Codes Output:",
                "New Codes Output with BID:": """[f'New Codes <BID {self.new_main_file.ModuleId.upper()}>:']""",
            ...
            "msg_map":
            {
                ...
                "Unpack Warning": """[f'Unpack "{Path(file_path).suffix}" takes time, please be patient.', '"keys.txt" in ROOT requested before use.']""",
                ...
                "NOT NSO File": """['Parsing error. NOT a valid NSO File.']""",
                ...
                "To many results": """[f'Too many results, address number will be limited to {self.max_shown_address}.']""",
 ...
 "loc_CN":
         {
            ...
            "hints_map":
            {
                ...
                "Input Old Codes with BID:": """[f'旧金手指 <BID {self.old_main_file.ModuleId.upper()}>：']""",
                "Current Processing Codes:": "当前处理金手指：",
                "Wing Length:": "翼展宽度：",
                "Logs:": "提示：",
                "New Codes Output:": "新金手指输出：",
                "New Codes Output with BID:": """[f'新金手指 <BID {self.new_main_file.ModuleId.upper()}>：']""",
            ...
            "msg_map":
            {
                ...
                ".nso extraction failed": """['从游戏包提取 "main" 文件失败']""",
                "Unpack Warning": """[f'解包 "{Path(file_path).suffix}" 文件需要一段时间，请耐心等待。', '使用前请确保根目录下存在"keys.txt"。']""",
                "Extract NCA": """['从游戏包提取NCA文件中......']""",
                "Extract ticket": """['从.tik获取相关信息中......']""",
                "Extract main": """['从.nca提取main文件中......']""",

                "NOT NSO File": """['解析错误，NSO文件不合法']""",
                ...
                "To many results": """[f'定位结果过多，将缩减地址数至{self.max_shown_address}']""",

...
class Progress_Bar:
	def __init__(self, ncz_file_size):
		self.root = tkinter.Tk()
		self.root.geometry('800x30')
		self.root.title('Decompressing NCZ File ...')
		self.left_text = tkinter.Label(self.root, text='Decompress  0%')
		...
		self.right_text = tkinter.Label(self.root, text=f'0/{ncz_file_size} MiB [00:00<00:00, 0.00 MiB/s]')
		self.right_text.pack(expand='yes', fill='both', anchor='w', side='right', padx=5, pady=5)
		self.progressbarOne['value'] = 0
		self.progressbarOne['maximum'] = ncz_file_size
		self.ncz_file_size = ncz_file_size
		self.timer_start = time.time()
		self.timer_stop = self.timer_start

	def update(self, decompressed_ncz_file_size):
		self.timer_stop = time.time()
		elapsed_time = self.timer_stop - self.timer_start
		if elapsed_time != 0:
			process_speed = decompressed_ncz_file_size/elapsed_time
			if process_speed != 0:
				eta_time = self.ncz_file_size/process_speed
				self.left_text.config(text=f'Decompress  {decompressed_ncz_file_size*100//self.ncz_file_size}%')
				self.progressbarOne['value'] = decompressed_ncz_file_size
				self.right_text.config(text=f'{decompressed_ncz_file_size}/{self.ncz_file_size} MiB [{time.strftime("%M:%S", time.localtime(elapsed_time))}<{time.strftime("%M:%S", time.localtime(eta_time))}, {"%.2f" % process_speed} MiB/s]')
				self.root.update()
...
def __decompressNcz(nspf, f, statusReportInfo, pleaseNoPrint):
	...
	if statusReportInfo == None:
		progress_bar = Progress_Bar(nca_size//1048576)
	...
	if statusReportInfo != None:
		...
		progress_bar.update(decompressedBytes//1048576)

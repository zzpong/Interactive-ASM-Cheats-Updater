...
class CodeUpdaterInterface:
    ...
    def update_middle_ASM_output(self, midASMDataContainer):
        ...
        if ((midASMDataContainer.is_view_target and midASMDataContainer.branch_target_size == 1)
            or (not midASMDataContainer.is_view_target and midASMDataContainer.branch_addr_size == 1)):
            self.btn_prev_addr.config(state=DISABLED)
            self.btn_next_addr.config(state=DISABLED)
    ...
    def get_code_title_by_pos(self, position: list):
        try:
            return self.code.code_struct[str(position[0])]['0']['contents']['raw'][0]
        except:
            return None
    ...
    def set_current_cheats_text_out(self, position: list):
        ...
        code_title = self.get_code_title_by_pos(position)
        ...
        if position[1] != 0 and code_title is not None:
            msg = code_title + '\n\n' + msg
    ...
    def broadcast_link_update(self, output_msg: str, cur_position: list, addr: int):
        ...
        for tar_position in broadcast_list:
            ...
            pattern = f"""C{tar_position[0]}C{tar_position[1]}BLC{cur_position[0]}C{cur_position[1]}O{offset}"""
    ...
    def sav_cht(self):
        ...
        with open(file = file_path, mode = 'a+', encoding = 'utf-8') as file:
            ...
            messagebox.showinfo(title='Info', message='\n'.join(eval(self.msg_map['Cheat Code Saved'])))
    ...
    def sav_nso(self):
        ...
        if file_path != '':
            with open(file = file_path, mode = 'wb') as file:
                ...
                messagebox.showinfo(title='Info', message='\n'.join(eval(self.msg_map['NSO File Saved'])))

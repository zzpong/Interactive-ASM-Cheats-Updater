...
class CodeStruct:
    ...
    def is_neighbor(self, addr_main, addr_extend: int):
        if not isinstance(addr_main, int):
            addr = addr_main[-1]
        return (addr_extend - addr == 4)  # TODO: Accelerate by recognizing reverse neighbor address codes
    ...
    def get_available_position(self, code_num: int, addr_list: list):
        ...
        for addr in addr_list:
            if code_num == addr[0] or self.code_struct[str(addr[0])]['0']['type'] == 'master_code_title':
